...
export default function ToolsPage() { return <><section className="page-hero"><div className="shell"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Tools" }]}/><div className="page-hero-grid"><div className="page-hero-copy"><span className="eyebrow">Practical applications</span><h1>Turn information into a plan</h1><p>Use interactive tools without creating an account. Assumptions are visible, progress stays on your device, and high-risk answers point back to official sources.</p></div><div className="page-stat"><span>Available now</span><strong>7</strong><small>Planning · costs · deadlines · cities · documents · arrival · housing safety</small></div></div></div></section><section className="section shell"><div className="tools-grid">{tools.map(([name, description, href, status]) => <Link className="tool-card" href={href} key={name}><span>{status}</span><h2>{name}</h2><p>{description}</p><em>Open tool <Icon name="arrow" size={16}/></em></Link>)}</div></section></> }
