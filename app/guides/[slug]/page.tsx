    ...
    <article>
      ...
      <div className="shell article-layout">
        ...
        <div className="article-body">
          ...
          {guide.sections.map((section, index) => <section className="article-section" id={toId(section.heading)} key={section.heading}><h2>{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}{section.callout && <aside className={`callout callout-${section.callout.tone || "note"}`}><strong>{section.callout.title}</strong><p>{section.callout.text}</p></aside>}{index === 1 && <AdSlot placement="article-body"/>}</section>)}
          <PartnerDisclosure/>
