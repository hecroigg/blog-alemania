...
export default function Home() {
  ...
  return <>
    ...
    <section className="section shell"><div className="section-heading"><div><span className="eyebrow">Plan your life in Germany</span><h2>Start with your real situation</h2></div><p>Move from generic reading to a personalised route, transparent budget and practical checklist.</p></div><div className="action-card-grid">{[
      ["🛂", "Check visa requirements", "Separate EU, EEA, Swiss and purpose-specific non-EU routes.", "/visas-residence"],
      ["💶", "Calculate living costs", "Build a range from your own rent, household and lifestyle.", "/tools/cost-of-living"],
      ["🗺", "Explore Germany", "Navigate all 16 states and practical city pathways.", "/explore-germany"],
      ["🏙", "Compare cities", "Understand trade-offs without an arbitrary ranking.", "/tools/compare-cities"],
      ["✅", "Build your checklist", "Save document progress locally, without an account.", "/tools/document-checklist"],
      ["🏠", "Find housing safely", "Learn the rental system and check common scam signals.", "/tools/rental-scam-checker"],
    ].map(([symbol, title, text, href]) => <Link href={href} className="action-card" key={title}><span>{symbol}</span><strong>{title}</strong><small>{text}</small><Icon name="arrow"/></Link>)}</div><div className="center-action"><Link className="button button-primary button-large" href="/plan">Build my personal Germany plan <Icon name="arrow"/></Link></div></section>

    <section className="section shell section-compact"><div className="section-heading"><div><span className="eyebrow">Living in Germany</span><h2>The systems behind everyday life</h2></div><p>Each hub connects the decisions, documents, and next steps around it.</p></div><div className="category-grid">{categories.map((category) => <CategoryCard key={category.slug} category={category}/>)}</div></section>
    ...
    <section className="section shell"><div className="section-heading"><div><span className="eyebrow">Tools & calculators</span><h2>Make the next decision concrete</h2></div><p>Transparent assumptions, local progress storage and clear limits.</p></div><div className="tool-preview"><div><Icon name="check"/><strong>Document checklist</strong><span>Generate and track the documents relevant to your route.</span></div><div><Icon name="wallet"/><strong>Cost-of-living calculator</strong><span>Build a budget from your real rent and inputs.</span></div><div><Icon name="city"/><strong>City comparison</strong><span>Compare practical trade-offs across 2–4 cities.</span></div><Link href="/tools">Open all tools <Icon name="arrow"/></Link></div></section>
